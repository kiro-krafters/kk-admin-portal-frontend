...
import { PhoneFabIcon, MinusIcon } from "../AdminShell/AdminIcons";
...
export default function FloatingCCP() {
  ...
  // Auto-open the window when a contact arrives.
  useEffect(() => {
    if (contact) {
      setOpen(true);
      setMinimized(false);
  ...
  // Re-clamp when the viewport resizes.
  useEffect(() => {
    function onResize() {
  ...
  if (minimized) {
    return (
      <button
        ...
        className="fixed bottom-6 right-6 z-40 flex items-center gap-2 rounded-full bg-connect-navy px-4 py-2 text-sm font-medium text-white shadow-connect-panel hover:bg-connect-navy-deep"
      >
        <PhoneFabIcon className="h-4 w-4 text-connect-teal" />
  ...
  return (
    ...
    >
      <div
        ...
        className="flex h-7 cursor-move items-center justify-between bg-connect-navy px-2 text-white"
      >
